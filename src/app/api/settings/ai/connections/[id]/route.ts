...
import { deleteAIConnection, updateAIConnection } from '@/lib/ai-config';
...
export async function PATCH(
  ...
  try {
    ...
    const connection = await updateAIConnection(id, {
      label,
    ...
    return apiError(
      message === 'Internal server error' ? 'Failed to update AI connection' : message,
      status,
      {
        details: process.env.NODE_ENV === 'production' ? undefined : details,
        logPrefix: '[PATCH /api/settings/ai/connections/[id]]',
        error,
      },
    );
  }
...
export async function DELETE(
  ...
  try {
    ...
    const { message, status, details } = classifyError(error);
    return apiError(
      message === 'Internal server error' ? 'Failed to delete AI connection' : message,
      status,
      {
        details: process.env.NODE_ENV === 'production' ? undefined : details,
        logPrefix: '[DELETE /api/settings/ai/connections/[id]]',
        error,
      },
    );
  }
